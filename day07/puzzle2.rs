...
impl Crab {
    ...
    fn fuel_to(&mut self, endpoint : i32) -> i32 {
        // fuel consumption is sum_i=1^n i = (1/2) * n * (n+1)
        let mut difference : i32 = endpoint - self.position;
        difference = i32::abs(difference);
        let math : f32 = 0.5*((difference*(difference+1)) as f32);
        return math as i32;
    }
}
...
fn main() {
    ...
    let mut dummy_crab = Crab::new(0);
    let mut minfuel : i32 = dummy_crab.fuel_to(maxpos)*(squad.len() as i32);
    for p in 0..maxpos+1 {
